...
echo "Step 10/16: Deploying GitHub emulator..."
bash "${SCRIPT_DIR}/07-deploy-github-emulator.sh"
echo ""

# Step 11: Deploy Jira emulator
echo "Step 11/16: Deploying Jira emulator..."
bash "${SCRIPT_DIR}/08-deploy-jira-emulator.sh"
echo ""

# Step 12: Deploy pipeline dashboard
echo "Step 12/16: Deploying pipeline dashboard..."
kubectl apply -f ${PROJECT_ROOT}/deploy/k8s/20-pipeline-dashboard.yaml
echo ""

# Step 13: Deploy MLflow
echo "Step 13/16: Deploying MLflow..."
bash "${SCRIPT_DIR}/10-deploy-mlflow.sh"
echo ""

# Step 14: Deploy markovd
echo "Step 14/16: Deploying markovd (workflow engine)..."
bash "${SCRIPT_DIR}/12-deploy-markovd.sh"
echo ""

# Step 15: Deploy ingress proxy
echo "Step 15/16: Deploying ingress proxy (Go reverse proxy)..."
bash "${SCRIPT_DIR}/09-deploy-ingress-proxy.sh"
echo ""

# Step 16: Wait for all deployments
echo "Step 16/16: Waiting for all deployments to be ready..."
