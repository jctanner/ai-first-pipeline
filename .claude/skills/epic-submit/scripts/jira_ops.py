#!/usr/bin/env python3
"""Jira REST API operations for epic-submit skill.
...
def cmd_link_issues(args):
    """Create a link between two Jira issues.

    Usage: jira_ops.py link-issues INWARD-KEY OUTWARD-KEY [--type TYPE]

    Default link type is "Blocks" (inward blocks outward).
    """
    if len(args) < 2:
        print("Usage: jira_ops.py link-issues INWARD-KEY OUTWARD-KEY [--type TYPE]",
              file=sys.stderr)
        sys.exit(1)
    ...
    api_version = "3" if _is_cloud(server) else "2"
    url = f"{server}/rest/api/{api_version}/issueLink"
    try:
        _request(url, user, token, server, body)
        print(json.dumps({"status": "linked", "inward": inward_key,
                          "outward": outward_key, "type": link_type}))
    except urllib.error.HTTPError as e:
        error_body = e.read().decode("utf-8", errors="replace")
        if e.code in (400, 409) and "already" in error_body.lower():
            print(json.dumps({"status": "already_linked", "inward": inward_key,
                              "outward": outward_key, "type": link_type}))
        else:
            print(f"HTTP {e.code}: {error_body}", file=sys.stderr)
            sys.exit(1)
