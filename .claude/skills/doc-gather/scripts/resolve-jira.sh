...
#
# Requires JIRA_USER (or JIRA_EMAIL) and JIRA_TOKEN environment variables (loaded from .env).
#
...
if [[ -z "${JIRA_AUTH_USER}" ]] || [[ -z "${JIRA_TOKEN:-}" ]]; then
    echo "Error: JIRA_USER (or JIRA_EMAIL) and JIRA_TOKEN must be set (add them to .env)." >&2
    exit 1
fi

# Fetch the issue
RESPONSE=$(curl -s -u "${JIRA_AUTH_USER}:${JIRA_TOKEN}" \
