...
def main() -> None:
    ...
    parser.add_argument("--resolve-version", metavar="VERSION", help="Resolve a Jira version string")
    ...
    if args.section:
        ...
        if args.section not in section_map:
            print(f"Error: unknown section '{args.section}'. Available: {', '.join(section_map)}", file=sys.stderr)
            sys.exit(1)
