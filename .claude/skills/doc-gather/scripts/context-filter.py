...
def stage_component_affinity(candidates: list[dict], task_context: dict, source_declarations: list[dict]) -> list[dict]:
    ...
    for c in candidates:
        ...
        if decl is None or decl.get("components") is None:
            c.setdefault("signals", []).append({
                ...
                "passed": True,
                "reason": f"No component affinity configured for source type '{c.get('source_type', '')}'",
            })
            result.append(c)
            continue
...
def stage_path_relevance(candidates: list[dict], task_context: dict, path_hints: list[str]) -> list[dict]:
    ...
    for rank, c in enumerate(sorted_candidates, start=1):
        ...
        if tokens_used + est_tokens <= token_budget:
            ...
            c.setdefault("signals", []).append({
                ...
                "passed": True,
                "reason": f"Selected at rank {rank} ({tokens_used + est_tokens}/{token_budget} tokens)" if not c.get("static") else "Static source (always selected first)",
            })
